...
def is_duplicate_title(title, json_filename):
    """检查标题是否已经在文件中存在，避免重复写入。"""
...
def fetch_post_content(post_url, json_filename, current_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 检查是否已经存在相同标题的内容
            if is_duplicate_title(title, json_filename):
                print(f"跳过已存在的文章：{title}")
                return True  # 标题重复，返回True表示应该翻页
            ...
            if content_div:
                ...
                print(f"已写入[{current_name}]的 {title}")
                return False  # 标题不重复，继续处理
        else:
...
def load_all_content(encoded_name, json_filename, max_results, current_name):
    """持续加载内容直到没有更多的新内容，处理翻页逻辑。"""
    next_page_url = f"{base_url}{encoded_name}?max-results={max_results}"

    while next_page_url:
        try:
            response = session.get(next_page_url, timeout=10, verify=False)
            if response.status_code == 200:
                soup = BeautifulSoup(response.text, 'html.parser')
                ...
                # 检查主页链接
                home_link = soup.find('a', {'class': 'home-link'})
                if home_link and "主页" in home_link.get_text():
                    # 如果有主页结构，但仍然有标题，那么继续
                    if not post_links:  # 没有标题，则停止
                        print(f"主页链接已找到，没有更多文章，停止翻页。")
                        break

                # 如果页面没有文章标题，则停止翻页
                if not post_links:
                    print(f"没有更多文章标题，停止翻页。")
                    break
                ...
                for post_link in post_links:
                    ...
                    if link_tag and 'href' in link_tag.attrs:
                        ...
                        # 如果标题重复，跳过并翻页
                        if fetch_post_content(post_url, json_filename, current_name):
                            print("检测到重复标题，继续翻页。")
                            break  # 跳过当前页，翻页继续

                # 翻页逻辑
                pager = soup.find('span', id='blog-pager-older-link')
                if pager:
                    next_page_tag = pager.find('a')
            ...
            print(f"请求内容时出错：{next_page_url}，错误信息：{e}")
            break
...
def fetch_links_and_contents(name):
    ...
    # 加载并处理所有内容
    load_all_content(encoded_name, json_filename, max_results, name)
...
def main():
    with ThreadPoolExecutor(max_workers=3) as executor:  # 最多3个线程同时执行
